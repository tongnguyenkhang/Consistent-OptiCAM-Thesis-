"""
ScoreCAM Implementation using pytorch_grad_cam library
======================================================
Uses official ScoreCAM from pytorch_grad_cam for better performance and reliability.
Includes checkpointing, plotting, and advanced metrics.

Usage:
    python generate_scorecam_v2.py --name_path "ScoreCAM_Baseline" --batch_size 10
"""
...
from cam_custom_targets import CatDogGroupTarget
...
# ==================== Flags ====================
flags.DEFINE_string('name_path', 'ScoreCAM', 'Name for results directory')
flags.DEFINE_integer('batch_size', 10, 'Batch size for processing')
...
def main(_):
    ...
    print(f"\n{'='*60}")
    print(f"ScoreCAM Configuration (pytorch_grad_cam library)")
    print(f"{'='*60}")
    ...
    # Initialize ScoreCAM (using pytorch_grad_cam library)
    ...
    # Cat/Dog indices (matching generate_opticam.py)
    try:
        from torchvision.models import ResNet50_Weights
    ...
    print(f"Note: Using pytorch_grad_cam library (optimized ScoreCAM)")
    print(f"{'='*60}\n")
    ...
    for batch_idx, (images, labels, file_names) in enumerate(loader):
        ...
        # Generate ScoreCAM saliency maps (using library)
        saliency_start = time.time()
        # Use custom target for cat/dog group aggregation
        if idx_cat.numel() > 0 and idx_dog.numel() > 0:
            targets = [CatDogGroupTarget(int(label.item()), idx_cat, idx_dog) for label in labels]
        else:
            targets = [ClassifierOutputTarget(int(label.item())) for label in labels]
        grayscale_cam = scorecam(input_tensor=images, targets=targets)  # Returns (B, H, W) normalized [0,1]
        ...
        for i in range(B):
            ...
            if any_adv_metric and used_flag:
                img_np = images[i].detach().cpu().numpy().transpose(1, 2, 0)  # (H, W, 3)
        ...
        if (batch_idx + 1) % 5 == 0 or (batch_idx + 1) == len(loader):
            ...
            print(f"Batch {batch_idx+1}/{len(loader)} | "
                  ...
                  f"Saliency time: {saliency_elapsed:.2f}s | "
    ...
    with open(summary_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("ScoreCAM Metrics Summary (pytorch_grad_cam library)\n")
        f.write("=" * 60 + "\n\n")
        ...
        f.write(f"Avg saliency time per image: {total_saliency_time/counted_samples:.2f}s\n\n")
    ...
    print(f"Avg per image: {total_wall_time/counted_samples:.2f}s")
