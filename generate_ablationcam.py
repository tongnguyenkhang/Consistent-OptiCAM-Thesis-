"""
Ablation-CAM Implementation using pytorch_grad_cam library
===========================================================
Uses official AblationCAM from pytorch_grad_cam for better performance and reliability.
Includes checkpointing, plotting, and advanced metrics.

Note: AblationCAM uses ablation (not gradient), may be slower than gradient-based methods.

Usage:
    python generate_ablationcam.py --name_path "AblationCAM_Baseline" --batch_size 3
"""
...
# ==================== Flags ====================
flags.DEFINE_string('name_path', 'AblationCAM', 'Name for results directory')
flags.DEFINE_integer('batch_size', 3, 'Batch size for processing (lower for AblationCAM)')
...
def main(_):
    ...
    print(f"\n{'='*60}")
    print(f"AblationCAM Configuration (pytorch_grad_cam library)")
    print(f"{'='*60}")
    ...
    print(f"\nNote: AblationCAM uses ablation (slower than gradient methods)")
    print(f"{'='*60}\n")
    ...
    # Initialize AblationCAM (using pytorch_grad_cam library)
    ...
    print(f"Note: Using pytorch_grad_cam library (optimized AblationCAM)")
    print(f"{'='*60}\n")
    ...
    for batch_idx, (images, labels, file_names) in enumerate(loader):
        ...
        # Generate AblationCAM saliency maps (using library)
        saliency_start = time.time()
        targets = [ClassifierOutputTarget(int(label.item())) for label in labels]
        grayscale_cam = ablationcam(input_tensor=images, targets=targets)  # Returns (B, H, W) normalized [0,1]
    ...
    with open(summary_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("AblationCAM Metrics Summary (pytorch_grad_cam library)\n")
        f.write("=" * 60 + "\n\n")
