"""
Layer-CAM Implementation using pytorch_grad_cam library
========================================================
Uses official Layer-CAM from pytorch_grad_cam for better performance and reliability.
Includes checkpointing, plotting, and advanced metrics.

Note: Layer-CAM is a single-pass method (1 forward + 1 backward), NO iterations needed.

Usage:
    python generate_layercam.py --name_path "LayerCAM_Baseline" --batch_size 5
"""
...
# ==================== Flags ====================
flags.DEFINE_string('name_path', 'LayerCAM', 'Name for results directory')
...
def main(_):
    ...
    print(f"\n{'='*60}")
    print(f"Layer-CAM Configuration (pytorch_grad_cam library)")
    print(f"{'='*60}")
    ...
    print(f"\nNote: Layer-CAM is single-pass (no iterations)")
    print(f"{'='*60}\n")
    ...
    # Initialize Layer-CAM (using pytorch_grad_cam library)
    ...
    print(f"Note: Using pytorch_grad_cam library (optimized Layer-CAM)")
    print(f"{'='*60}\n")
    ...
    for batch_idx, (images, labels, file_names) in enumerate(loader):
        ...
        # Generate Layer-CAM saliency maps (using library)
        saliency_start = time.time()
        targets = [ClassifierOutputTarget(int(label.item())) for label in labels]
        grayscale_cam = layercam(input_tensor=images, targets=targets)  # Returns (B, H, W) normalized [0,1]
    ...
    with open(summary_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("Layer-CAM Metrics Summary (pytorch_grad_cam library)\n")
        f.write("=" * 60 + "\n\n")
