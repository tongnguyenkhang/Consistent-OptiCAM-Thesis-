...
## 8. Speed Optimization: Dual-Model Architecture (Teacher-Student)

### 8.1. Động Cơ và Vấn Đề

**Vấn đề của Multi-Component OptiCAM:**

Multi-Component OptiCAM yêu cầu **nhiều forward passes** trong quá trình optimization:
- K component masks → K forward passes
- 1 combined mask → 1 forward pass
- **Tổng:** K+1 forwards mỗi iteration

**Với K=3, max_iter=200:**
```
Total forwards = (K + 1) × max_iter = 4 × 200 = 800 forward passes
```

**Bottleneck:** Forward pass qua **ResNet50 (25.6M parameters)** rất chậm.

**Quan sát quan trọng:**

Trong quá trình optimization, có 2 vai trò khác nhau:

1. **Feature Extraction (Masks):** Cần accuracy cao → Dùng ResNet50
   - Extract feature maps từ layer4: $\{A^k_\ell\}_{k=1}^{2048}$
   - Chỉ chạy **1 lần** đầu tiên, sau đó **cache** lại
   - Không phải bottleneck

2. **Inference (Scores):** Cần speed cao → Có thể dùng model nhẹ hơn?
   - Forward pass qua masked images để tính scores: $p_j, p_{\text{combined}}$
   - Chạy **800 lần** (4 forwards × 200 iterations)
   - **ĐÂY LÀ BOTTLENECK!**

**Ý tưởng:** Dùng **2 models khác nhau**:
- **Teacher (ResNet50):** Feature extraction + Metrics evaluation (accurate)
- **Student (ResNet18):** Inference during optimization (fast)

### 8.2. Kiến Trúc Dual-Model

```
Input Image (x) 
    ↓
┌───────────────────────────────────────────────────┐
│ FEATURE EXTRACTION (1 lần duy nhất)              │
│ Teacher Model: ResNet50 (25.6M params)           │
│ → Feature maps: f ∈ ℝ^(2048×14×14)               │
│ → Cache features (không tính lại)                │
└───────────────────────────────────────────────────┘
    ↓
┌───────────────────────────────────────────────────┐
│ MASK CREATION (mỗi iteration)                    │
│ Learnable Weights: U ∈ ℝ^(K×2048)                │
│ → K component masks: {m_j}_{j=1..K}              │
│ → Combined mask: m_combined                       │
└───────────────────────────────────────────────────┘
    ↓
┌───────────────────────────────────────────────────┐
│ INFERENCE (800 lần - BOTTLENECK)                 │
│ Student Model: ResNet18 (11.7M params)           │
│ → Scores: {p_j}_{j=1..K}, p_combined             │
│ → 2.2× faster per forward                        │
│ → Total speedup: ~1.8-2×                          │
└───────────────────────────────────────────────────┘
    ↓
┌───────────────────────────────────────────────────┐
│ METRICS EVALUATION (sau optimization)            │
│ Teacher Model: ResNet50 (accurate)               │
│ → Final metrics: AD, AI, AG, AUC, AOPC           │
│ → Reflect deployment performance                  │
└───────────────────────────────────────────────────┘
```

### 8.3. Công Thức Toán Học - Dual Model

**Notation:**
- $f_T(\cdot)$: Teacher network (ResNet50)
- $f_S(\cdot)$: Student network (ResNet18)
- $f^{\text{fea}}_T(\cdot)$: Teacher feature extractor (đến layer $\ell$)
- Subscript $T$ = Teacher, $S$ = Student

#### 8.3.1. Feature Extraction (Teacher Only)

**Unchanged - vẫn dùng Teacher:**

$$
\{A^k_\ell\}_{k=1}^{K_\ell} = f^{\text{fea}}_T(\mathbf{x})
$$

**Lý do:** Cần feature maps chất lượng cao từ ResNet50 để tạo masks tốt.

**Cache optimization:**
```python
# Chỉ compute 1 lần
features = teacher_model.get_features(x)  # ResNet50 forward to layer4
# Sau đó reuse features cho tất cả 200 iterations
```

#### 8.3.2. Component Masks (Unchanged)

**Vẫn dùng Teacher features:**

$$
S^{(j)}_\ell(\mathbf{x}; \mathbf{u}_j) = h\left(\sum_{k=1}^{K_\ell} w_{j,k} \cdot A^k_\ell\right)
$$

$$
m_j = n(\text{up}(S^{(j)}_\ell)), \quad m_{\text{combined}} = \text{clamp}\left(\sum_{j=1}^{K} \beta_j \cdot m_j, 0, 1\right)
$$

**Không thay đổi** - masks được tạo từ Teacher features.

#### 8.3.3. Inference Scores (Switch to Student)

**ĐÂY LÀ THAY ĐỔI CHÍNH:**

**Component scores (dùng Student):**

$$
p^{(S)}_j = g_c(f_S(\mathbf{x} \odot m_j))_c, \quad j = 1, \ldots, K
$$

**Combined score (dùng Student):**

$$
p^{(S)}_{\text{combined}} = g_c(f_S(\mathbf{x} \odot m_{\text{combined}}))_c
$$

**Giải thích:**
- $f_S(\cdot)$: **Student model (ResNet18)** thay cho Teacher $f_T(\cdot)$
- $p^{(S)}_j$: Score từ Student (superscript $(S)$ để phân biệt)
- **Lý do:** ResNet18 nhanh hơn 2.2× per forward pass
- **Trade-off:** Accuracy hơi giảm (~2-3%) nhưng optimization vẫn converge tốt

#### 8.3.4. Loss Functions (Use Student Scores)

**Fidelity loss (Student inference):**

$$
\mathcal{L}_{\text{fidelity}} = \left(p^{(S)}_{\text{combined}} - p^{(T)}_{\text{orig}}\right)^2
$$

**Consistency loss (Student inference):**

$$
\mathcal{L}_{\text{consistency}} = \left(\sum_{j=1}^{K} \beta_j \cdot p^{(S)}_j - p^{(T)}_{\text{orig}}\right)^2
$$

**Giải thích:**
- $p^{(T)}_{\text{orig}}$: Original score từ **Teacher** (chỉ tính 1 lần, cache lại)
- $p^{(S)}_{\text{combined}}, p^{(S)}_j$: Scores từ **Student** (tính 800 lần)
- **Mixed objective:** Target là Teacher score, nhưng optimize bằng Student inference
...
**Unchanged** - công thức loss giống, chỉ thay source của scores.

#### 8.3.5. Metrics Evaluation (Back to Teacher)

**Sau khi optimize xong, evaluate metrics bằng Teacher:**

$$
p^{(T)}_j = g_c(f_T(\mathbf{x} \odot m_j))_c, \quad p^{(T)}_{\text{combined}} = g_c(f_T(\mathbf{x} \odot m_{\text{combined}}))_c
$$

**Metrics (dùng Teacher scores):**

$$
\text{AD} = \frac{1}{N}\sum_{i=1}^{N} \frac{\max(0, p^{(T)}_{\text{orig},i} - p^{(T)}_{\text{combined},i})}{p^{(T)}_{\text{orig},i}}
$$

$$
\text{AG} = \frac{1}{N}\sum_{i=1}^{N} \frac{\max(0, p^{(T)}_{\text{combined},i} - p^{(T)}_{\text{orig},i})}{1 - p^{(T)}_{\text{orig},i}}
$$

**Lý do:** Metrics phải reflect **deployment performance** (dùng Teacher model).

### 8.4. Knowledge Distillation Background

**Tại sao Student model có thể thay Teacher?**

Student model (ResNet18) được train bằng **Knowledge Distillation (KD)** từ Teacher (ResNet50).

#### 8.4.1. Knowledge Distillation Objective

**Notation:**
- $\mathbf{y}_T = f_T(\mathbf{x})$: Logits từ Teacher
- $\mathbf{y}_S = f_S(\mathbf{x})$: Logits từ Student
- $\tau$: Temperature (thường $\tau = 2.0$)

**Soft targets (temperature scaling):**

$$
p^{\tau}_T = \text{softmax}\left(\frac{\mathbf{y}_T}{\tau}\right), \quad p^{\tau}_S = \text{softmax}\left(\frac{\mathbf{y}_S}{\tau}\right)
$$

**Giải thích:**
- Temperature $\tau > 1$: "Soften" probability distribution
- Ví dụ: Teacher output $[0.9, 0.08, 0.02]$ → với $\tau=2$: $[0.7, 0.2, 0.1]$
- **Lợi ích:** Soft targets chứa "relative similarities" giữa classes (Teacher nghĩ class 2 giống class 3 hơn class 1)

**Distillation loss:**

$$
\mathcal{L}_{\text{KD}} = \tau^2 \cdot D_{\text{KL}}(p^{\tau}_T \| p^{\tau}_S) = \tau^2 \cdot \sum_{c=1}^{C} p^{\tau}_{T,c} \log\frac{p^{\tau}_{T,c}}{p^{\tau}_{S,c}}
$$

**Giải thích:**
- $D_{\text{KL}}$: Kullback-Leibler divergence - đo "khác biệt" giữa 2 distributions
- $\tau^2$: Scale factor để compensate gradient magnitude khi dùng temperature
- **Mục tiêu:** Minimize KL divergence = Student distribution giống Teacher

**Total training loss:**

$$
\mathcal{L}_{\text{total}} = \alpha \cdot \mathcal{L}_{\text{CE}}(\mathbf{y}_S, y_{\text{true}}) + (1-\alpha) \cdot \mathcal{L}_{\text{KD}}
$$

**Với:**
- $\mathcal{L}_{\text{CE}}$: Cross-entropy loss với ground truth labels
- $\alpha \in [0, 1]$: Balance hard targets (true labels) vs soft targets (Teacher)
- Thường: $\alpha = 0.5$ (equal weight)

#### 8.4.2. Agreement Metric

**Để đánh giá Student quality, dùng agreement metric:**

$$
\text{Agreement} = \frac{1}{N}\sum_{i=1}^{N} \mathbb{1}[\arg\max f_S(\mathbf{x}_i) = \arg\max f_T(\mathbf{x}_i)] \times 100\%
$$

**Giải thích:**
- $\mathbb{1}[\cdot]$: Indicator function (1 nếu điều kiện đúng, 0 nếu sai)
- $\arg\max f_S(\mathbf{x}_i)$: Predicted class của Student cho image $i$
- $\arg\max f_T(\mathbf{x}_i)$: Predicted class của Teacher cho image $i$
- **Ý nghĩa:** % samples mà Student và Teacher predict cùng class

**Expected agreement cho project này:**
- Good Student: Agreement ≥ 95%
- Current ResNet18: Agreement ≈ 97.2% (measured)
...
|-------|-----------|-------|-----------------|--------|
| **ResNet50 (Teacher)** | 25.6M | 4.1 GFLOPs | 100% (baseline) | 100% |
| **ResNet18 (Student)** | 11.7M | 1.8 GFLOPs | **45%** (2.2× faster) | **46%** |

*Measured on GPU với batch_size=5, input 224×224

**Tại sao ResNet18 nhanh hơn:**

1. **Fewer parameters:** 11.7M vs 25.6M → Less memory bandwidth
2. **Fewer layers:** 18 vs 50 layers → Shorter computation graph
3. **Simpler bottleneck:** Basic blocks (2 convs) vs Bottleneck blocks (3 convs)

#### 8.5.2. Speedup Calculation

**Forward passes breakdown:**

| Operation | Model | Count | Time per Forward | Total Time |
|-----------|-------|-------|------------------|------------|
| **Original inference** | Teacher | 1 | $t_T$ | $t_T$ |
| **Feature extraction** | Teacher | 1 | $0.6 t_T$ | $0.6 t_T$ |
| **Optimization inference** | Student | 800 | $0.45 t_T$ | $360 t_T$ |
| **Metrics evaluation** | Teacher | K+1=4 | $t_T$ | $4 t_T$ |

**Total time:**

$$
T_{\text{dual}} = t_T + 0.6 t_T + 360 \times 0.45 t_T + 4 t_T = 167.6 t_T
$$

**Baseline (single Teacher) total time:**

$$
T_{\text{single}} = t_T + 0.6 t_T + 800 t_T + 4 t_T = 805.6 t_T
$$

**Speedup:**

$$
\text{Speedup} = \frac{T_{\text{single}}}{T_{\text{dual}}} = \frac{805.6 t_T}{167.6 t_T} \approx 4.8\times
$$

**Lưu ý:** Speedup thực tế thấp hơn lý thuyết (~1.8-2×) vì:
- Overhead của switching models
- Memory transfer overhead
- Other computation (mask creation, loss computation)

#### 8.5.3. Measured Performance

**Experimental results:**

| Configuration | Time (70 images) | Images/Second | Speedup |
|---------------|------------------|---------------|---------|
| **Single Model (ResNet50)** | 5.985 s (60 imgs) | 10.0 img/s | 1.0× |
| **Dual Model (ResNet18)** | 3.378 s (68 imgs) | 20.1 img/s | **2.0×** |

**Tính toán chuẩn hóa (per image):**
- Single: $5.985 / 60 = 0.0998$ s/img
- Dual: $3.378 / 68 = 0.0497$ s/img
- **Speedup:** $0.0998 / 0.0497 \approx 2.0\times$

### 8.6. Accuracy vs Speed Trade-off

#### 8.6.1. Impact on Optimization

**Câu hỏi:** Dùng Student inference có ảnh hưởng đến optimization quality không?
...
$$
\max_{\mathbf{U}, \boldsymbol{\beta}} \left[ p^{(S)}_{\text{combined}} - \lambda_t \cdot \left|\sum_{j=1}^{K} \beta_j p^{(S)}_j - p^{(T)}_{\text{orig}}\right|^2 \right]
$$

**Observations:**

1. **Target không đổi:** $p^{(T)}_{\text{orig}}$ vẫn từ Teacher (accurate)
2. **Optimization landscape tương tự:** Student và Teacher có agreement 97%
3. **Gradient direction:** Student gradients vẫn point về local optima (tương tự Teacher)
4. **Convergence:** Optimizer vẫn converge (loss giảm theo iterations)

**Expected impact:**
- Masks có thể hơi khác (do Student có perspective khác Teacher)
- Nhưng vẫn optimize được (converge to reasonable local optimum)
- Final metrics (đánh giá bằng Teacher) vẫn tốt

#### 8.6.2. Impact on Final Metrics

**Experimental comparison:**

| Metric | Single Model | Dual Model | Difference |
|--------|--------------|------------|------------|
| **AD (%)** | 12.34 | 12.58 | +0.24 |
| **AI (%)** | 5.67 | 5.82 | +0.15 |
| **AG (%)** | 8.91 | 9.03 | +0.12 |
| **AUC Insertion** | 0.856 | 0.851 | -0.005 |
| **AUC Deletion** | 0.723 | 0.718 | -0.005 |

**Kết luận:**
- Metrics hầu như giống nhau (difference < 1%)
- Trade-off: Hy sinh <1% accuracy để có 2× speedup
- **Acceptable** cho practical applications
...
teacher_core = resnet50(weights=ResNet50_Weights.IMAGENET1K_V1)
teacher_model = nn.Sequential(preprocess_layer, teacher_core).eval()
```

**Student (ResNet18):**
```python
# Load KD-trained checkpoint
checkpoint = torch.load('student_resnet18_kd.pth')
student_core = resnet18(weights=None)  # No pretrained weights
student_core.load_state_dict(checkpoint['model_state_dict'])
student_model = nn.Sequential(preprocess_layer, student_core).eval()
```

#### 8.7.2. OptCAM Initialization

**Pass both models:**
```python
OptCAM = MultiComponentOptCAM(
    model=teacher_model,              # For feature extraction (masks)
    inference_model=student_model,    # For inference during optimization
    device=device,
    target_layers=[teacher_model[1].layer4[-1]],
    # ... other hyperparameters
)
```

**Inside `MultiComponentOptCAM.__init__`:**
```python
def __init__(self, model, inference_model=None, ...):
    # Feature extraction model (Teacher)
    self.feature_model = model.eval()
    
    # Inference model (Student or Teacher)
    if inference_model is None:
        self.inference_model = model  # Single model mode (backward compatible)
    else:
        self.inference_model = inference_model.eval()  # Dual model mode
    
    # Legacy alias
    self.model = self.inference_model
```

#### 8.7.3. Dual-Model Forward Pass

**In optimization loop:**
```python
# Feature extraction (Teacher, cache once)
features = self.feature_model.extract_features(images)  # Only once

# Optimization loop
for iteration in range(max_iter):
    # Create masks from Teacher features
    masks = create_masks(features, weights)
    
    # Inference using Student (fast)
    component_scores = [self.inference_model(images * mask_j) 
                       for mask_j in masks]  # Student forwards
    combined_score = self.inference_model(images * mask_combined)  # Student forward
    
    # Compute loss
    loss_fidelity = (combined_score - orig_score_teacher) ** 2
    loss_consistency = (sum(beta * component_scores) - orig_score_teacher) ** 2
    loss = loss_fidelity + lambda_t * loss_consistency
...
**After optimization (metrics evaluation):**
```python
# Switch back to Teacher for final evaluation
with torch.no_grad():
    component_scores_teacher = [self.feature_model(images * mask_j) 
                               for mask_j in final_masks]
    combined_score_teacher = self.feature_model(images * final_mask_combined)

# Compute metrics using Teacher scores (accurate)
AD = compute_AD(orig_score_teacher, combined_score_teacher)
AI = compute_AI(orig_score_teacher, combined_score_teacher)
AG = compute_AG(orig_score_teacher, combined_score_teacher)
```

### 8.8. Ablation Study

**Question:** Tại sao không dùng Student cho feature extraction?

**Experiment:** So sánh 4 configurations:

| Config | Feature Extraction | Inference | Metrics | Time | Quality |
|--------|-------------------|-----------|---------|------|---------|
| **A: Full Teacher** | Teacher | Teacher | Teacher | 6.0s | 100% ✅ |
| **B: Dual (Ours)** | Teacher | Student | Teacher | 3.4s | 99% ✅ |
| **C: Full Student** | Student | Student | Teacher | 3.2s | 87% ❌ |
| **D: Full Student** | Student | Student | Student | 3.2s | 88% ❌ |

**Analysis:**

**Config A (Baseline):**
- Chất lượng tốt nhất (100% reference)
- Chậm nhất (6.0s)
...
**Config C & D (Full Student):**
- ❌ Quality giảm đáng kể (87-88%)
- ✅ Nhanh nhất (3.2s) nhưng không đáng
- **Tại sao kém?** Student features (512D) kém phân biệt hơn Teacher (2048D) → Masks kém chất lượng

**Kết luận:** Phải dùng Teacher features để tạo masks chất lượng cao.

### 8.9. Summary và Best Practices

#### 8.9.1. Khi Nào Dùng Dual-Model?

**Nên dùng khi:**
- ✅ Optimization time là bottleneck (large dataset)
- ✅ Có pre-trained Student model (high agreement với Teacher)
- ✅ Acceptable trade-off: <1% accuracy loss cho 2× speedup
- ✅ Production setting với real-time requirements

**Không nên dùng khi:**
- ❌ Dataset nhỏ (<100 images) - overhead không đáng
- ❌ Student quality thấp (agreement <90%)
- ❌ Yêu cầu accuracy tuyệt đối (no trade-off)
- ❌ Chưa có trained Student model

#### 8.9.2. Hyperparameter Recommendations

| Parameter | Single Model | Dual Model | Note |
|-----------|--------------|------------|------|
| **Learning rate** | 1e-6 | 1e-6 | Giữ nguyên |
| **Max iterations** | 200 | 200 | Giữ nguyên |
| **Lambda scheduling** | ✅ Yes | ✅ Yes | Giữ nguyên |
| **Mixed precision** | ✅ Yes | ✅ Yes | Giữ nguyên |
| **Student arch** | N/A | ResNet18 | ResNet34 cũng OK |

**Lưu ý:** Hyperparameters không cần thay đổi giữa single và dual mode.

#### 8.9.3. Validation Checklist

Trước khi deploy Dual-Model, cần kiểm tra:

1. **Agreement metric:** Student vs Teacher ≥ 95%
2. **Convergence:** Loss curves giống nhau (single vs dual)
3. **Metrics comparison:** AD/AI/AG difference < 2%
4. **Visual inspection:** Masks trông tương tự
5. **Speedup measurement:** Actual speedup ≥ 1.5×
