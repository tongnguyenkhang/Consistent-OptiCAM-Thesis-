"""
GradCAM Implementation using pytorch_grad_cam library
=====================================================
Uses official GradCAM from pytorch_grad_cam for better performance and reliability.
Includes checkpointing, plotting, and advanced metrics.

Note: GradCAM is a single-pass method (1 forward + 1 backward), NO iterations needed.

Usage:
    python generate_gradcam_v2.py --name_path "GradCAM_Baseline" --batch_size 5
"""
...
# ==================== Flags ====================
flags.DEFINE_string('name_path', 'GradCAM', 'Name for results directory')
...
def main(_):
    ...
    print(f"\n{'='*60}")
    print(f"GradCAM Configuration (pytorch_grad_cam library)")
    print(f"{'='*60}")
    ...
    print(f"\nNote: GradCAM is single-pass (no iterations)")
    print(f"{'='*60}\n")
    ...
    # Initialize GradCAM (using pytorch_grad_cam library)
    ...
    print(f"Note: Using pytorch_grad_cam library (optimized GradCAM)")
    print(f"{'='*60}\n")
    ...
    for batch_idx, (images, labels, file_names) in enumerate(loader):
        ...
        # Generate GradCAM saliency maps (using library)
        ...
        grayscale_cam = gradcam(input_tensor=images, targets=targets)  # Returns (B, H, W) normalized [0,1]
    ...
    with open(summary_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("GradCAM Metrics Summary (pytorch_grad_cam library)\n")
        f.write("=" * 60 + "\n\n")
